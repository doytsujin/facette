<template>
    <div class="v-graph-tooltip">
        <table>
            ...
            <tbody>
                <tr :class="{null: entry.value === null}" :key="idx" v-for="(entry, idx) in tooltip.entries">
                    <th>
                        <span class="color" :style="{backgroundColor: entry.color}"></span>
                        {{ entry.name }}
                    </th>
                    <td>{{ entry.value !== null ? entry.value : "null" }}</td>
                </tr>
            </tbody>
        </table>
    </div>
        ...
        update(e, chart) {
            ...
            let tooltip = {
                ...
                entries: chart.config.series.map((series) => {
                    ...
                    return {
                        name: series.name,
                        value: value !== null ? chart.config.axes.y.ticks.format(value) : null,
                        color: series.color,
                    };
                }),
            };
